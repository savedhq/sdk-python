...
class CreateClientStorageJson(BaseModel):
    """
    CreateClientStorageJson
    """ # noqa: E501
    name: StrictStr
    provider: StrictStr
    config: Dict[str, Any] = Field(description="Provider-specific configuration (S3 or FTP)")
    __properties: ClassVar[List[str]] = ["name", "provider", "config"]

    @field_validator('provider')
    def provider_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['s3', 'ftp']):
            raise ValueError("must be one of enum values ('s3', 'ftp')")
        return value
